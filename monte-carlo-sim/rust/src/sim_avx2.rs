...
impl State {
    pub fn new(allocator: &mut Bump, teams: &[TeamDto]) -> Self {
        unsafe {
            ...
            // since we iterate through and load these numbers into 256bit vectors.
            ...
            // Now vectorization, so doesn't need to be multiple of 4
...
pub fn simulate<'a, const S: usize>(state: &mut State, markets_allocator: &'a mut Bump) -> Markets<'a> {
    unsafe {
        ...
        assert!(state.poisson_len % LANES == 0);
...
unsafe fn tick(rng: &mut RngImpl, home_poisson: *mut f64, away_poisson: *mut f64, table: *mut f64, len: u32) {
    for i in 0..len {
        ...
        let home_poisson = _mm256_set1_pd(*home_poisson.add(i as usize));
        let mut home_points = _mm256_setzero_pd();
        ...
        while j < len {
            ...
            let away_poisson = _mm256_load_pd(away_poisson.add(j as usize));

            let mut home_goals = _mm256_setzero_pd();
            let mut away_goals = _mm256_setzero_pd();
            ...
            let home = _mm256_cmp_pd::<_CMP_GT_OQ>(home_goals, away_goals);
            // home_goals == away_goals
            let draw = _mm256_cmp_pd::<_CMP_EQ_OQ>(home_goals, away_goals);
            // home_goals < away_goals
            let away = _mm256_cmp_pd::<_CMP_LT_OQ>(home_goals, away_goals);

            // Still need to mask of the i == j case
            let draw_mask = _mm256_andnot_pd(exclude_mask, draw);

            // Home
            let home_mask = _mm256_andnot_pd(exclude_mask, home);
            // Conditionally adds points based on the mask - points are only added when the respective lane bit is set
            home_points = _mm256_add_pd(home_points, _mm256_and_pd(_mm256_set1_pd(3.0), home_mask));
            home_points = _mm256_add_pd(home_points, _mm256_and_pd(_mm256_set1_pd(1.0), draw_mask));

            // Away
            let mut away_points = _mm256_setzero_pd();
            let away_mask = _mm256_andnot_pd(exclude_mask, away);
            away_points = _mm256_add_pd(away_points, _mm256_and_pd(_mm256_set1_pd(3.0), away_mask));
            away_points = _mm256_add_pd(away_points, _mm256_and_pd(_mm256_set1_pd(1.0), draw_mask));

            // Write the points for the away teams back to the table by doing load, add, store
            let table_section = _mm256_load_pd(table.add(j as usize));
            _mm256_store_pd(table.add(j as usize), _mm256_add_pd(table_section, away_points));

            j += LANES;
        }

        // Horizontally add the home vector
        *table.add(i as usize) += mm256_reduce_add_pd(home_points);
    }
}

// Taken from https://stackoverflow.com/a/49943540
unsafe fn mm256_reduce_add_pd(v: __m256d) -> f64 {
...
unsafe fn simulate_sides(L: __m256d, k: &mut __m256d, rng: &mut RngImpl) {
    /*
    algorithm poisson random number (Knuth):
        ...
        return k − 1.
    */
    let mut p = rng.next_m256d();

    loop {
        let sub = _mm256_sub_pd(p, L);
        ...
        *k = _mm256_add_pd(*k, _mm256_round_pd::<0x02>(sub));
        // *k = _mm256_mask_add_pd(*k, !mask, *k, _mm256_set1_pd(1.0)); // slower perf

        let next_product_vec = rng.next_m256d();
        p = _mm256_mul_pd(p, next_product_vec);
    }
}
