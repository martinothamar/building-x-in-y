...
impl Node {
    ...
    fn get_gossip_messages_for(&self, node: &str) -> Vec<u64> {
        ...
        let filter = |m: &u64| !self.messages_pending_by_value.contains(&(*m, node.to_string()));
        match node_messages {
            Some(set) => self.messages.difference(set).copied().filter(filter).collect(),
            None => self.messages.iter().copied().filter(filter).collect(),
        }
    }
                ...
                if is_new {
                    ...
                    for neighbor in neighbors {
                        ...
                        if messages_to_broadcast.is_empty() {
                            if DEBUG {
                                logger.log(format!(
                                    "[{}] skipped neighbor - already knows everything, or messages are pending: {:?} {:?}\n",
                                    self.id(),
                                    neighbor,
                                    &message
                                ));
